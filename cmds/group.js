...
function formatTimestamp(date = new Date()) {
  ...
  return `${month} ${day}${suffix} ${year}, ${hour}:${minute}:${second} ${ampm}`;
}

// Example usage:
const now = formatTimestamp();
console.log(now);
// Output: July 12th 2025, 6:08:45 PM
...
kord({ cmd: "hack|trace|blacktrace", desc: "Realistic hacking prank simulation", fromMe: wtype, type: "group" }, async (m, text) => { try { let target = text || m.mentionedJid?.[0] || m.reply_message?.sender || "Unknown User"; if (typeof target === "object") target = target.jid;

await m.send(`💻 Initializing BlackTrace Protocol...`);
await delay(1000);
await m.send(`🔍 Target acquired: @${target.split("@")[0]}`);

const device = getDevice(m.key.id);
await delay(1000);
await m.send(`📱 Device Info: *${device}*, Android v${Math.floor(Math.random() * 5 + 10)}.0`);
await delay(1000);

const ip = await getPublicIP();
await m.send(`🌐 Public IP Trace: ${ip}`);
await delay(1000);

const fakeCoords = `6.${Math.floor(Math.random() * 100000)}, 3.${Math.floor(Math.random() * 100000)}`;
await m.send(`📍 Triangulating location coordinates...

🧭 Approximate Location: ${fakeCoords}`); await delay(1500);

await m.send(`📡 Injecting backdoor into WhatsApp client...`);
await delay(1300);
await m.send(`📂 Accessing DCIM/photos...

📦 Downloading: 97 files`); await delay(1200);

await m.send(`🧠 Running facial scan analysis...`);
await delay(1800);
await m.send(`📷 Snap retrieved! (Image redacted for privacy)`);
await delay(1000);

await m.send(`📄 Extracting Chat.db...

💬 Dumping last 10 chat lines...); await delay(1500); await m.send(🧾 Logs: "heyy😂😂" "😂you serious?" "drop location asap ." "seen." "Coming..."`);

await delay(1500);
await m.send(`⚠️ Intrusion Alert! Retaliation Detected...`);
await delay(1000);

await m.send(`🔁 Reversing trace to: *YOU* (@${m.sender.split("@")[0]})

💣 Wiping traces... 🕳️ Injecting data bomb...`); await delay(2000);

await m.send(`💥 *DATA EXPLOSION COMPLETE!*

🧨 Session scrambled. ☠️ BlackTrace has reached root layer.`);

await delay(1500);
const time = formatTimestamp();
await m.send(`🛑 *▂▃▄▅▆▇█▓▒░ 😈☠️💀 ░▒▓█▇▆▅▄▃▂ *

Hack executed at ${time}! 💻 Your device is now breached.

Stay sharp, @${target.split("@")[0]} 😉`, { mentions: [target] });

} catch (err) { console.log("Hack prank error:", err); await m.sendErr(err); } });
